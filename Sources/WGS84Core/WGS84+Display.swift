...
extension Coordinate2D: GeographicNotation {
	...
	@inlinable public func ddNotation(maxDigits: UInt8) -> String {
		let lat = self.latitude.ddNotation(maxDigits: maxDigits)
		let long = self.longitude.ddNotation(maxDigits: maxDigits)
		return "\(lat), \(long)"
	}

	@inlinable public func dmNotation(full: Bool = false, maxDigits: UInt8) -> String {
	...
	@inlinable public func dmsNotation(full: Bool = false, maxDigits: UInt8) -> String {
